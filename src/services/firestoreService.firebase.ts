...
export const recordRedemption = async (
    ...
    return createEntryLedgerItem(
        {
            getEntriesByPass: getRedemptionsByPass,
            appendEntry: async (entry) => {
                await addDoc(collection(db, 'redemptions'), entry as RedemptionDocument);
            },
        },
        { passId, dealName, vendorId, userId, userEmail, userName },
        {
            onFirstEntry: ({ userEmail, userName, dealName }) => {
                if (!userEmail) return;
                return triggerFirstRedemptionEmail(userEmail, userName, dealName);
            },
        }
    );
};
